...
if __name__ == "__main__":
    ...
    arr = prediction.flatten().tolist()
    for i in range(len(arr)):
        if arr[i] < 0.05:
            abnormal.add(lstm_idx[i][0])

    with open("result_abnormal_macaddress.json", "w") as f:
        f.write(json.dumps(list(abnormal)))
    print("The output json file has already created. Check result_abnormal_macaddress.json for details.")